...
Auf dem Screen (siehe \figref{fig:latest-builds}) sind diese Komponenten nach Datum absteigend sortiert. Ein größerer Abstand und die deutlich sichtbaren durchgezogenen Farbbalken hilft bei der Trennung der Komponenten voneinander, damit der Fokus auf der einzelne Komponente bleibt.
...
\texttt{dispatch()} in Zeile 4 ist eine Method aus Redux (siehe \fullref{subsec:react-redux}). Durch sie werden Änderungen an den globalen State übermittelt. Die Methode \texttt{fetchProject()} startet eine Anfrage an die API mit der entsprechenden Projekt-ID.

Im globalen State wird das Feld \texttt{project.isFetching} während der Anfrage auf \texttt{true} gesetzt. Sind die Daten angekommen, wird sie auf \texttt{false} gesetzt. Damit kann in entweder ein Ladeindikator angezeigt werden, oder die verschachtelte Komponente aufgerufen werden (siehe \lstref{lst:project-render}).
...
Die Komponente \texttt{Project} soll verschiedene Screens anzeigen (vgl. Auflistung in \fullref{subsec:entwurf-user-interface}). Je nach URL wird die Historie, die neuesten Build-Prozesse oder die Details zu einem Build-Prozess angezeigt.
...
Dieser Ablauf ist im Action Creator aus \lstref{lst:project-async-action} auch so implementiert. (Im Action Creator \texttt{receiveProject()} in \lstref{lst:project-action-creators} ist das Hinzufügen der Daten und Entfernen des Hinweises vereint.)
