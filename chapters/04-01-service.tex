...
Für den Service wurden die in \tabref{tab:rest-routes} definierten Endpunkte für eine RESTful API ausgearbeitet. Als Datenformat wurde JSON gewählt. Dies eignet sich besonders für die Verwendung in einem JavaScript-basierten Frontend Client.
...
Die Zugehörigkeit zwischen Pipeline in der Datenbank und in der Konfigurationsdatei wird über einen Identifikator bestimmt. (\texttt{konfiguration\_\allowbreak inidentifikator} in \figref{fig:uml} bzw. \texttt{human\_id} in der letztendlichen Implementation, siehe \fullref{subsec:api-usage})
...
In der Datenstruktur (\figref{fig:uml}) entspricht dem Feld \emph{git\_ref\_auslöser}.
